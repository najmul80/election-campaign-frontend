...
const filteredPrograms = computed(() => {
  ...
  return programsArray.filter((p) => {
    ...
    const matchConstituency = selectedConstituency.value
      ? p.constituency === selectedConstituency.value
      : true
    return matchDate && matchCandidate && matchConstituency
  })
})
</script>
...
<template>
  ...
  >
    <div class="container mx-auto px-4">
      ...
      <!-- Filter Bar -->
      <div
        class="flex flex-wrap gap-4 mb-6 justify-center bg-gray-50 dark:bg-gray-800 p-4 rounded-xl shadow"
      >
        <!-- Date -->
        <div
          class="flex items-center border border-green-300 dark:border-green-700 rounded-lg px-3 py-2 text-sm focus-within:ring-2 focus-within:ring-green-500 dark:bg-gray-900 dark:text-white transition"
        >
          <span class="material-icons text-green-500 mr-2">calendar_today</span>
          <input type="date" v-model="selectedDate" class="flex-1 bg-transparent outline-none" />
        </div>
        ...
        <div
          class="flex items-center border border-green-300 dark:border-green-700 rounded-lg px-3 py-2 text-sm focus-within:ring-2 focus-within:ring-green-500 dark:bg-gray-900 dark:text-white transition"
        >
          <span class="material-icons text-green-500 mr-2">person</span>
          <select v-model="selectedCandidate" class="flex-1 bg-transparent outline-none">
            <option value="">সব প্রার্থী</option>
            <option
              v-for="p in [...new Set(programs.map((pr) => pr.candidate))]"
              :key="p"
              :value="p"
            >
              {{ p }}
        ...
        >
          ...
          <select v-model="selectedConstituency" class="flex-1 bg-transparent outline-none">
            <option value="">সব আসন</option>
            <option
              v-for="c in [...new Set(programs.map((pr) => pr.constituency))]"
              :key="c"
              :value="c"
            >
              {{ c }}
      ...
      <div v-else class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        >
          ...
          <div class="h-48 overflow-hidden relative bg-gray-200 dark:bg-gray-700 flex-shrink-0">
            <img
              :src="program.image || 'https://placehold.co/600x400?text=Event'"
              :alt="program.title"
            ...
            <!-- Date Badge -->
            <div
              class="absolute top-4 right-4 bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm px-3 py-1 rounded-lg text-center shadow"
            >
          ...
          <div class="p-6 flex flex-col flex-1">
            <h3
              class="text-xl font-bold mb-3 text-gray-800 dark:text-white group-hover:text-green-600 dark:group-hover:text-green-400 transition line-clamp-2 h-[3.5rem]"
            >
              {{ program.title }}
            </h3>

            <div class="flex items-center text-gray-500 dark:text-gray-400 mb-4 text-sm">
              <span class="material-icons text-green-500 dark:text-green-400 mr-2 text-base"
                >location_on</span
              >
      ...
      <!-- Empty State -->
      <div v-if="!loading && filteredPrograms.length === 0" class="text-center py-10">
